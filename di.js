/* di.js — DiCo Rhythm / ARU (God-Game Edition) [FULL REWRITE]
   ✅ “神ゲー”仕様：気持ちよさ最優先のテンポ設計＋演出レイヤ安全＋iOS安定
   - Tap / Rapid / Burst(3連) / Hold(長押し) + Release判定
   - FEVER：100%到達で自動発動（短い・強い・戻りも気持ちいい）
   - TRIAD（COMPASS/TRIGGER/ROUTE）：スコア到達で展開 → 成功3回でGOD
...
(() => {
  ...
  const CFG = Object.freeze({
    HIT_Y_RATIO: 0.78,          // CSSターゲットと揃える（stage::after が 78%）
    NOTE_SPEED:  610,           // 速め＝爽快（iOSでも耐える範囲）
    NOTE_RADIUS: 12,

    BPM: 145,
    BEAT: 60 / 145,

    // 判定（秒）— 神ゲーの肝：狭すぎず、気持ちよく上達できる
    ...
    FEVER_MAX: 100,
    FEVER_DURATION: 6.2,
    FEVER_DECAY_AFTER: 70,      // 発動後に戻す値（連発しすぎない＆気持ちよい）
  ...
  async function primeMedia() {
    ...
    // iOS: playsInline + muted を先に
    video.playsInline = true;
    video.muted = true;

    // “ユーザー操作中”に一度だけ再生→停止で権限確保
  ...
  function setProtocolUI(step) {
    ...
    if (protocolStack) {
      if (S.protocolStep === 0) protocolStack.classList.remove("on");
      else protocolStack.classList.add("on");
    }

    if (p1) p1.dataset.state = S.protocolStep >= 1 ? (S.protocolStep >= 2 ? "done" : "on") : "idle";
    if (p2) p2.dataset.state = S.protocolStep >= 2 ? (S.protocolStep >= 3 ? "done" : "on") : "idle";
    if (p3) p3.dataset.state = S.protocolStep >= 3 ? "on" : "idle";
  ...
  function findClosestNote(t, allowHoldStart = true) {
    // 近い範囲だけ探索（noteIndex中心）
    let best = null;
    let closest = 999;

    const win = judgeWindow();
    const scan = win.good + 0.20;  // 探索余裕
