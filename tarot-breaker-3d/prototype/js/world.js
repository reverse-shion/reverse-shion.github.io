...
export function createWorld(canvas, map, events) {
  ...
  // One small graybox. These primitive shapes are technical placeholders.
  ...
  // The visible low balustrade coincides with the collision boundary.
  ...
  // A simple lintel and closed doorway suggest the area around Lumière's room.
  ...
  // Inlaid lines provide movement and distance cues without a game minimap.
  ...
  // A restrained static star backdrop, not the Phase 3 sky / environment system.
  const stars = [];
  let seed = 37;
  const rand = () => { seed = (seed * 16807) % 2147483647; return seed / 2147483647; };
  for (let i = 0; i < 180; i++) { const a = rand() * Math.PI * 2, y = rand() * 0.8 + 0.16, r = Math.sqrt(1 - y * y) * 32; stars.push(Math.sin(a) * r, y * 32, Math.cos(a) * r); }
  const starsGeometry = new THREE.BufferGeometry(); starsGeometry.setAttribute('position', new THREE.Float32BufferAttribute(stars, 3));
  ...
  function render(player) {
    if (disposed) return;
    camera.position.set(player.x, map.player.eyeHeight, player.z);
