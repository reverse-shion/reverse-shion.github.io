(function (root) {
  ...
  const authoredOccluders = () => (layout.occluders || []).filter((area) =>
    area && area.source === "foreground" && !area.depthFix &&
    Array.isArray(area.points) && area.points.length >= 3
  );
  ...
  layout.artworkPlacement = Object.freeze({
    ...
    foreground: Object.freeze({ mode: "single-map-occlusion", x: 0, y: 0, w: reference.width, h: reference.height, scale: 1 }),
  });
